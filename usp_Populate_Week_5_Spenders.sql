...
IF OBJECT_ID(N'usp_Week_5_Spenders',N'P') IS NOT NULL
	DROP PROCEDURE usp_Week_5_Spenders;
GO
CREATE PROCEDURE usp_Week_5_Spenders ( @DebugMode BIT )    
AS
...
BEGIN TRY  
         ...
         IF @DebugMode = 0
         BEGIN
				IF OBJECT_ID(N'Spend_By_Customer_Week_5',N'U') IS NOT NULL
				DROP TABLE Spend_By_Customer_Week_5;

				CREATE TABLE Spend_By_Customer_Week_5 ( Cust_ID BIGINT
				                                     ,[WeekOfMonthName] VARCHAR(50)
								                     ,AvgSpend FLOAT
								                    );

				INSERT INTO Spend_By_Customer_Week_5 ( Cust_ID
				                                    ,[WeekOfMonthName]
								                    ,AvgSpend
								                   )
				SELECT T.CustomerID
				...
				IF OBJECT_ID(N'Week_5_Data',N'U') IS NOT NULL
				DROP TABLE Week_5_Data;
				CREATE TABLE Week_5_Data ( Cust_ID BIGINT
								          ,Week_5_Spend FLOAT
								         );

				INSERT INTO Week_5_Data ( Cust_ID
				                         ,Week_5_Spend
									    )
				SELECT T.CustomerID
					  ,AVG(T.Amount) AS Week_5_Spend 
				FROM tbl_TransformedEvents T 
				WHERE T.eventType = 'WITHDRAWAL'
				AND T.accountType = 'CURRENT' 
				AND Amount < (	
				...
				FROM Spend_By_Customer_Week_5 Tw
				GROUP BY Tw.Cust_ID 


				TRUNCATE TABLE tbl_Week_5_Spenders;

				INSERT INTO tbl_Week_5_Spenders([CustomerID])
				SELECT Customer_MaxSpend_By_Week.Cust_ID
				FROM Customer_MaxSpend_By_Week 
				INNER JOIN Week_5_Data ON Customer_MaxSpend_By_Week.Cust_ID = Week_5_Data.Cust_ID
				AND Customer_MaxSpend_By_Week.AvgSpend_Max = Week_5_Data.Week_5_Spend 
         END
